...
async def start_handler(msg:Message, state:FSMContext):
    ...
    await msg.answer(text='Привет. Меня зовут Hike Helper. Я помогу тебе разработать меню для твоего похода и рассчитаю все необходимые для него продукты. Выбери, что будем делать дальше:',
                     reply_markup=mrkp)
...
async def process_startdate(qry:CallbackQuery, state:FSMContext):
    ...
    try:
        ...
        await qry.message.answer(f'Произошла чудовищная ошибка в process_startdate!\n{e}')
    else:
...
async def process_enddate(qry:CallbackQuery, state:FSMContext):
    ...
    try:
        ...
        await qry.message.answer(f'Произошла чудовищная ошибка в process_enddate!\n{e}')
    else:
...
async def process_firstfood(qry:CallbackQuery, state:FSMContext):
    ...
    try:
        ...
        await qry.message.answer(f'Произошла чудовищная ошибка в process_firstfood:\n{e}')
    else:
...
async def process_lastfood(qry:CallbackQuery, state:FSMContext):
    ...
    try:
        ...
        lenght=utils.callback_date_converter(data['enddate'])-utils.callback_date_converter(data['startdate'])
        ...
        if response:
            ...
            for row in response:
                ...
                if lastfood=='1':
                    ...
                    res_s='ужин' 
                count+=1
                records.append('Запись '+ str(count)+':\n ID записи - '+ str(row['id'])+ '; дата начала похода - '+ str(row['startdate'])+ '; дата окончания похода - '+ str(row['enddate'])+ ';\n'+'первый прием пищи - '+ res+'; последний прием пищи - '+ res_s +'\n')
...
async def menu_handler(qry:CallbackQuery):
    ...
    row=[[btn_create], [btn_show], [btn_menu]]
    mrkp=InlineKeyboardMarkup(inline_keyboard=row)
    await qry.message.answer(text='Что будем делать дальше?', reply_markup=mrkp)
