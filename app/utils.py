...
def calendar(year, month):
    ...
    for day in week:
        row.append(
            InlineKeyboardButton(text=f'{day}', callback_data='weekday')
        )
    rows.append(row)
    row = []
    ...
    for day in range(1, days_amount+1):
        if month < 10 and day < 10:
            row.append(InlineKeyboardButton(
                text=str(day), callback_data=f'{year}-0{month}-0{day}'
            ))
        elif month < 10:
            row.append(InlineKeyboardButton(
                text=str(day), callback_data=f'{year}-0{month}-{day}'
            ))
        elif day < 10:
            row.append(InlineKeyboardButton(
                text=str(day), callback_data=f'{year}-{month}-0{day}'
            ))
        else:
            row.append(InlineKeyboardButton(
                text=str(day), callback_data=f'{year}-{month}-{day}'
            ))
        if len(row) == 7:
            rows.append(row)
            row = []
...
def months_creator(buttons_in_row):
    months = ['январь',
              'февраль',
              'март',
              'апрель',
    ...
    row = [InlineKeyboardButton(
        text=day, callback_data=str(days_number)
    ) for day, days_number in zip(months, range(1, len(months)+1))]
    rows = []
    rows_fin = []
    for button in row:
        rows.append(button)
...
# вытаскивает tg_id из таблицы users
# (пока функциолнал не нужный, но может быть нужен потом, хз)
def uid_creator(checker, conn, uid):
